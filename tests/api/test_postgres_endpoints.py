    ...
    with patch("api.endpoints.postgres.create_engine", return_value=mock_engine):
        with patch("api.endpoints.postgres.inspect", return_value=mock_inspector):
             with TestClient(app) as client:
                 resp = client.get("/api/system/postgres/schemas")
...
def test_list_tables(monkeypatch):
    ...
    with patch("api.endpoints.postgres.create_engine", return_value=mock_engine):
        with patch("api.endpoints.postgres.inspect", return_value=mock_inspector):
             with TestClient(app) as client:
                 resp = client.get("/api/system/postgres/schemas/public/tables")
                 
    assert resp.status_code == 200
    assert resp.json() == ["table1", "table2"]

def test_list_tables_404_schema(monkeypatch):
    mock_engine = MagicMock()
    mock_inspector = MagicMock()
    mock_inspector.get_schema_names.return_value = ["public"]
    ...
    with patch("api.endpoints.postgres.create_engine", return_value=mock_engine):
        with patch("api.endpoints.postgres.inspect", return_value=mock_inspector):
             with TestClient(app) as client:
                 resp = client.get("/api/system/postgres/schemas/missing_schema/tables")
                 
    assert resp.status_code == 404
    assert "not found" in resp.json()["detail"]

def test_query_table_success(monkeypatch):
    mock_engine = MagicMock()
    mock_inspector = MagicMock()
    mock_inspector.get_schema_names.return_value = ["public"]
    ...
    with patch("api.endpoints.postgres.create_engine", return_value=mock_engine):
        with patch("api.endpoints.postgres.inspect", return_value=mock_inspector):
             with patch("api.endpoints.postgres.pd.read_sql", return_value=df) as mock_read:
                 with TestClient(app) as client:
                     resp = client.post("/api/system/postgres/query", json={
                         "schema_name": "public",
...
def test_query_table_security_fail(monkeypatch):
    mock_engine = MagicMock()
    mock_inspector = MagicMock()
    mock_inspector.get_schema_names.return_value = ["public"]
    mock_inspector.get_table_names.return_value = ["test_table"]
