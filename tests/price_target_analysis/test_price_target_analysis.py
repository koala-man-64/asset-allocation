...
@patch('scripts.price_target_analysis.price_target_analysis.mdc')
def test_transform_symbol_data(mock_mdc):
    ...
    # Verify save called
    # expected path: Price Targets/TEST.parquet
    mock_mdc.store_parquet.assert_called()
    args, _ = mock_mdc.store_parquet.call_args
    assert "Price Targets/TEST.parquet" in args[1]

@patch('scripts.price_target_analysis.price_target_analysis.mdc')
@patch('scripts.price_target_analysis.price_target_analysis.nasdaqdatalink')
