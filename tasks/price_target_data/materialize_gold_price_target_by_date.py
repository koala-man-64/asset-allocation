...
def _build_config(argv: Optional[List[str]]) -> MaterializeConfig:
    ...
    parser.add_argument(
        "--output-path",
        default=DataPaths.get_gold_price_targets_by_date_path(),
...
def discover_year_months_from_data(
    *, container: Optional[str] = None, max_tickers: Optional[int] = None
    ...
    tickers_from_container = _try_load_tickers_from_container(container, root_prefix="targets")
    ...
    if not tickers:
        write_line(
            f"No per-ticker price target feature tables found (source={ticker_source}); no year_months discovered."
        )
        return []
    ...
    write_line(
        f"Discovered {len(discovered)} year_month(s) from gold price target data in {container}."
    )
    return discovered
