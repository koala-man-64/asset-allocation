...
def _resolve_container(container_raw: Optional[str]) -> str:
    container_raw = container_raw or os.environ.get("AZURE_FOLDER_TARGETS")
...
def discover_year_months_from_data(
    *, container: Optional[str] = None, max_tickers: Optional[int] = None
    ...
    write_line(f"Discovered {len(discovered)} year_month(s) from silver price target data in {container}.")
    return discovered
...
def materialize_silver_targets_by_date(cfg: MaterializeConfig) -> int:
    ...
    tickers_from_container = _try_load_tickers_from_container(cfg.container, root_prefix="price-target-data")
    if tickers_from_container is None:
        tickers = _load_ticker_universe()
        ticker_source = "symbol_universe"
    else:
    ...
    if not tickers:
        write_line(f"No per-ticker price target tables found (source={ticker_source}); nothing to materialize.")
        return 0
