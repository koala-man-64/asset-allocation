...
function PipelineNode({ layer, domain, label }: PipelineNodeProps) {
    ...
    const statusColor =
        report.status === 'error' ? 'text-rose-600' :
            report.status === 'empty' ? 'text-amber-600' :
                'text-emerald-600';
        ...
        <div className="dq-node-card group">
            <div className="dq-node-header">
                <div className="flex items-center gap-2">
                    {layer === 'bronze' ? <FileBox className="h-3 w-3 text-muted-foreground" /> :
                        layer === 'silver' ? <TableIcon className="h-3 w-3 text-muted-foreground" /> :
            ...
            {report.columns && report.columns.length > 0 && (
                <div className="dq-detail-overlay hidden group-hover:block absolute top-[100%] left-0 right-0 z-10 shadow-lg ring-1 ring-border">
                    <div className="text-[10px] font-semibold uppercase text-muted-foreground mb-1">Column Stats</div>
