...
import { MarketData } from '@/types/data'; // Ensure this type exists or use local shim
import {
    Table,
    TableBody,
    TableCell,
...
export function StockExplorerPage() {
    ...
    const [watchlist, setWatchlist] = useState<WatchlistStock[]>(
        WATCHLIST.map(s => ({ symbol: s, price: null, change: null, changePercent: null, volume: null, loading: true, error: false }))
    );
        ...
        WATCHLIST.forEach(async (symbol) => {
            try {
                ...
                if (data && data.length > 0) {
                    ...
                    setWatchlist(prevList => prevList.map(item => item.symbol === symbol ? { ...item, loading: false, error: true } : item));
                }
            } catch (err) {
                setWatchlist(prevList => prevList.map(item => item.symbol === symbol ? { ...item, loading: false, error: true } : item));
            }
            ...
            <div className="flex-1 p-6 grid grid-cols-12 gap-6 overflow-y-auto">
                ...
                <div className="col-span-12 grid grid-cols-1 md:grid-cols-4 gap-4">
                    {watchlist.slice(0, 4).map(stock => (
                        <Card key={stock.symbol} className="border-l-4 border-l-primary/50 shadow-sm hover:shadow-md transition-shadow cursor-pointer" onClick={() => navigate(`/stock/${stock.symbol}`)}>
