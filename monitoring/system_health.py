    ...
    if item.memory_limit_bytes and "memorypercent" not in signal_names:
        ...
        if memory_signal is not None and memory_value is not None:
            ...
            enriched_signals.append(
                _build_percent_signal("MemoryPercent", memory_signal, memory_percent)
            )
...
def _supports_retry_symbol_metadata(job_name: str) -> bool:
    return str(job_name or "").strip().lower() in _RETRY_SYMBOL_METADATA_JOB_NAMES


def _parse_retry_symbol_metadata(message: str) -> Optional[Dict[str, Any]]:
    text = str(message or "").strip()
    if not text or _RETRY_SYMBOL_LOG_MESSAGE not in text:
        return None
...
def _query_job_retry_symbol_metadata(
    ...
    *,
    ...
    job_kql = _escape_kql_literal(job_name)
    exec_kql = _escape_kql_literal(str(execution_name or ""))
    message_kql = _escape_kql_literal(_RETRY_SYMBOL_LOG_MESSAGE)
    end = end_time or _utc_now()
    start = start_time or (end - timedelta(minutes=30))
    if end < start:
        end = _utc_now()
                ...
                def _enrich_resource(
                    ...
                    *,
                    ...
                    if monitor_metrics_enabled and metric_names and item.azure_id:
                        ...
                        if metric_signals:
                            metric_signals = _append_job_usage_percent_signals(item, metric_signals)
