...
def _process_ticker(task: Tuple[str, str, str, str]) -> Dict[str, Any]:
    ...
    ticker, raw_path, gold_path, market_container = task

    df_raw = delta_core.load_delta(market_container, raw_path)
    ...
    try:
        delta_core.store_delta(df_features, market_container, gold_path, mode="overwrite")
    except Exception as exc:
...
def _build_job_config() -> FeatureJobConfig:
    market_container = os.environ.get("AZURE_CONTAINER_MARKET") or ""
    if not market_container:
        raise RuntimeError("Missing required environment variable: AZURE_CONTAINER_MARKET")
...
def main() -> int:
    ...
    for ticker in job_cfg.tickers:
        ...
        tasks.append((ticker, raw_path, gold_path, job_cfg.market_container))
