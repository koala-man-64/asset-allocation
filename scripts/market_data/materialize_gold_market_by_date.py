...
def _build_config(argv: Optional[List[str]]) -> MaterializeConfig:
    ...
    parser.add_argument("--container", help="Market features container (default: AZURE_CONTAINER_MARKET).")
    parser.add_argument("--year-month", required=True, help="Year-month partition to materialize (YYYY-MM).")
    parser.add_argument(
        "--output-path",
        default=DataPaths.get_gold_features_by_date_path(),
...
def materialize_market_by_date(cfg: MaterializeConfig) -> int:
    ...
    tickers_from_container = _try_load_tickers_from_container(cfg.container, root_prefix="market")
    ...
    write_line(
        f"Materializing market_by_date for {cfg.year_month}: container={cfg.container} "
        f"tickers={len(tickers)} ticker_source={ticker_source} output_path={cfg.output_path}"
    )

    if not tickers:
        write_line(f"No per-ticker market feature tables found (source={ticker_source}); nothing to materialize.")
        return 0
    ...
    if not frames:
        write_line(f"No market feature rows found for {cfg.year_month}; nothing to materialize.")
        return 0
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    return materialize_market_by_date(cfg)
