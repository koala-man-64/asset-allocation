...
def _process_ticker(task: Tuple[str, str, str, str]) -> Dict[str, Any]:
    ...
    ticker, raw_path, gold_path, earnings_container = task

    df_raw = delta_core.load_delta(earnings_container, raw_path)
    ...
    try:
        delta_core.store_delta(df_features, earnings_container, gold_path, mode="overwrite")
    except Exception as exc:
...
def _build_job_config() -> FeatureJobConfig:
    earnings_container = os.environ.get("AZURE_CONTAINER_EARNINGS") or ""
    if not earnings_container:
        raise RuntimeError("Missing required environment variable: AZURE_CONTAINER_EARNINGS")
    ...
    return FeatureJobConfig(
        earnings_container=earnings_container,
...
def main() -> int:
    ...
    for ticker in job_cfg.tickers:
        ...
        tasks.append((ticker, raw_path, gold_path, job_cfg.earnings_container))
