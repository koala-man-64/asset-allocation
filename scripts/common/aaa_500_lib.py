...
def find_short_term(df_symbols: pd.DataFrame, df_combined: pd.DataFrame, strat: Strategy):
    try:
        ...
        for index, row in df_symbols.iterrows():
            ...
            #     continue
            found = True
            df_subset = df_combined[df_combined['Symbol'] == row['Symbol']]
            df_subset = remove_dupes(df_subset)
            
            if len(df_subset) <= 0:
                continue
            
            # Set 'Date' as the index
            df_subset = df_subset.tail(180)
            latest_date = df_subset['Date'].iloc[-1]
            df_subset.set_index('Date', inplace=True)
            recent_closes = df_subset['Close'].tail(3).values
            if row['Symbol'] == 'MGAM':
                d = ''
            # Check if the most recent value is greater than all of the previous 5
            is_greater = is_higher_than_all_past_n_days(df_subset, n=7, percentage_increase=0.05)
            if is_greater:
                # Plot the data

                write_line(f'Popped -> {row["Symbol"]}')
                mpf.plot(df_subset, type='candle', volume=True, style='yahoo', title=f'{row["Symbol"]} as of {latest_date.strftime("%Y-%m-%d")}')
                # df_subset.reset_index(inplace=True)
                plt.pause(0.001)
            else:
                write_inline(f'Not popped - {row["Symbol"]}')
            # Check if the most recent value is greater than all of the previous 5
            is_lower = is_lower_than_all_past_n_days(df_subset, n=7, percentage_increase=0.05)
            if is_lower:
                # Plot the data

                write_line(f'Dropped -> {row["Symbol"]}')
                mpf.plot(df_subset, type='candle', volume=True, style='yahoo', title=f'{row["Symbol"]} as of {latest_date.strftime("%Y-%m-%d")}')
                # df_subset.reset_index(inplace=True)
                plt.pause(0.001)
            else:
                write_inline(f'Not dropped - {row["Symbol"]}')
...
async def refresh_stock_data_async(df_symbols, lookback_bars, drop_prior, get_latest, browser, page, context):
    ...
    if not skip_reload:
        ...
        # 2) Try cache
        historical_path     = Path(r"G:/My Drive/Python/Common/get_historical_data_output.csv")
...
def refresh_stock_data2(df_symbols, lookback_bars, drop_prior, get_latest, page):
    # refresh df_combined
    skip_reload = False    
    if not skip_reload:
        write_line('Retrieving historical data...')
        ...
        df_ust = get_historical_data('UST', drop_prior, get_latest, page)[0]
        df_spy = get_historical_data('SPY', drop_prior, get_latest, page)[0]
...
def evaluate_current(df_symbols: pd.DataFrame, df_combined: pd.DataFrame, strat: Strategy, print:bool) -> pd.DataFrame:
    ...
    while True:
        try:
            df_top_combined.to_csv('G:\My Drive\Python\Common\df_top_combined.csv')
            break
        except Exception as e:
...
def get_symbols():
    ...
    file_path = 'G:\My Drive\Python\Common/df_symbols.csv'
    ...
    blacklist_path = 'G:\\My Drive\\Python\\Common\\blacklist.csv'
    blacklist_financial_path = 'G:\\My Drive\\Python\\Common\\blacklist_financial.csv'
    ...
    store_csv(df_symbols, file_path)
    pd.DataFrame(tickers_to_add).to_csv('G:\My Drive\Python\Common/market_analysis_tickers.csv', index=False)
    df_symbols.to_csv('G:\My Drive\Python\Common/stock_tickers.csv', index=False)
    return df_symbols
...
async def get_historical_data_async(ticker, drop_prior, get_latest, page) -> tuple[pd.DataFrame, str]:
    ...
    ticker_file_path = f'G:\My Drive\Python\Common\Yahoo\Price Data\{ticker}.csv'    
    ...
    while True:
        ...
        if os.path.exists(ticker_file_path) and len(matching_rows) > 0:
            ...
            try:
                ...
                path = download_path
                black_path = 'G:\My Drive\Python\Common/blacklist.csv'
                # white_path = 'G:\My Drive\Python\Common/whitelist.csv'
