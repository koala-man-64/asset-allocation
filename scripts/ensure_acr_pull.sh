...
for _ in $(seq 1 10); do
  case "$kind" in
    app)
      principal_id="$(az containerapp show --name "$resource_name" --resource-group "$resource_group" --query identity.principalId -o tsv 2>/dev/null || true)"
      ;;
    job)
      principal_id="$(az containerapp job show --name "$resource_name" --resource-group "$resource_group" --query identity.principalId -o tsv 2>/dev/null || true)"
      ;;
    *)
...
existing="$(az role assignment list --assignee "$principal_id" --scope "$acr_id" --query "[?roleDefinitionName=='AcrPull'] | length(@)" -o tsv --only-show-errors || true)"
existing="${existing:-0}"

if [ "$existing" = "0" ]; then
  az role assignment create --assignee "$principal_id" --role "AcrPull" --scope "$acr_id" --only-show-errors 1>/dev/null
