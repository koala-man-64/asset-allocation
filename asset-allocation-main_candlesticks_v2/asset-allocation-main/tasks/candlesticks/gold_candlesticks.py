"""Gold Candlestick Indicators (built from Silver market data).

This job derives classic candlestick pattern *indicators* from the cleaned OHLCV
data in the Silver layer (market-data/<ticker>) and writes the results as a
Gold per-ticker Delta table (candlesticks/<ticker>), plus a cross-sectional
table (candlesticks_by_date) partitioned by year_month and date.
...
def _build_materialize_config(argv: Optional[List[str]]) -> MaterializeConfig:
    ...
    parser.add_argument(
        "--output-path",
        default=DataPaths.get_gold_candlesticks_by_date_path(),
